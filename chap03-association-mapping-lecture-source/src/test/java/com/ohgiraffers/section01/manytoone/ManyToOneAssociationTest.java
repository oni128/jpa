...
public class ManyToOneAssociationTest {
     ...
     *     - N:1(ManyToOne) 연관관계 -> 그나마 이친구를 사용
     *     - 1:N(OneToMany) 연관관계 -> jpa 설계할때 지양
     *     - 1:1(OneToOne) 연관관계 -> XXX
     *     - N:N(ManyToMany) 연관관계 -> XXX
     *
     * 설명.
     *  2. 방향에 따른 분류
...
//        Category category = foundMenu.getCategory(); /*너가 가져온 그 상태에서 category 필요해*/
//        System.out.println("category = " + category);

        /* 설명. fetch=FetchType.LAZY로 두고 toString()에서 category 관련 코드도
