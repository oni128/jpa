...
//        Menu foundMenu3 = entityManager.find(Menu.class, menuCode);
    ...
    public void 새로운_메뉴_추가_테스트() {
        ...
        EntityTransaction transaction = entityManager.getTransaction(); /*dml작업할때 커밋과 롤백이 있어야 가능하다는걸 알 수 있음.*/
        transaction.begin();
    ...
    public void 메뉴_이름_수정_테스트() {
        Menu menu = entityManager.find(Menu.class, 2); /*2번에 없으면 select 있으면 관리된 데이터 가져오기?*/
        ...
        try {
            menu.setMenuName(menuNameToChange); /* update 문*/
            transaction.commit();
        } catch (Exception e) {
            transaction.rollback();
        }
